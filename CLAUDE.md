...
- **되살리는 열이 저장하는 열보다 적으면 재실행이 지운다** — `fetch_briefings`가 `news`·`anomaly`·`flow`를 복원하지 않던 시절, 멱등 재실행이 `persist`로 그것들을 덮어써 **15종목의 데이터가 실제로 사라졌다**(2026-08-30). `to_row()`에 열을 늘리면 `fetch_briefings`도 함께 늘린다 — `tests/test_store.py`의 왕복 테스트가 잡는다.
