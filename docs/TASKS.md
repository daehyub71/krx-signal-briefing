...
- [ ] 상위 `krx-stock-charts` **KRX 로그인 실패**(08-24부터 Actions에서만) 원인 확인 — 수급 수집이 여기 달려 있다 (R22)
...
- [ ] **상위 `krx-stock-charts` SPEC F14 신설** — `ksc_investor_flows(d, ticker, investor, net_value, net_volume)` · pykrx `get_market_net_purchases_of_equities_by_ticker` 시장2×투자자3 = 하루 6회. **사용자 승인 후 상위 리포에 구현**
