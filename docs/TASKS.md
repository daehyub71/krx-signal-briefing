...
| M4 자동화·배포 | `█████████░` | 92% | 12/13 | 🔄 |
| M5 마무리 | `████████░░` | 80% | 4/5 | 🔄 |
| **M6 신호 검증 (v3.0)** | `██████████` | 97% | 30/31 | 🔄 |
| **전체** | `██████████` | **97%** | **104/107** | 🔄 M4·M5·M6 |
...
| **M1b 전** | **D12·D13·D15 확정** (SPEC §2-3). D14는 ✅ 포함 확정(2026-08-29) | ✅ 2026-08-29 |
...
| M4 | fine-grained PAT (대상 `krx-signal-briefing` 1개 · Contents write) → 상위 리포 Secrets `BRIEFING_DISPATCH_TOKEN` | ✅ 2026-08-31 · dispatch 204 실동작 확인 |
| M4 | 이 리포 Secrets 등록 (최종 12종) | ✅ 2026-08-31 |
...
- [ ] **상위 워크플로의 진짜 실패 원인은 따로 있었다** — 08-27로 수동 실행하니 `fetch_daily_since`가 티커 2,700개를 `.in_()`에 한 번에 넣어 **PostgREST 400**. 일봉 쓰기는 그 앞이라 데이터는 남았지만 **주·월봉 재계산과 08/28 일봉이 빠졌고**, 그 예외가 F14(수급)까지 막았다. 상위에서 300개씩 나누도록 수정·테스트 완료
  - **08-18~08-31 피해 전부 복구 (2026-08-31)** — 일봉 08/18~08/28 전 거래일 2,767종목 · 주·월봉 08/28까지 재계산 · 시총 08/28 · 수급 84,492행 · **드리프트 소급 변경 12종목 재백필**. 08/28 일봉이 없는 2종목(시스웍 08/24~, 위지윅스튜디오 08/14~)은 **거래정지**로 결함이 아니다
  - **연쇄가 풀린 것을 실증** — 상위가 08-30·08-31에 `stale_data`(신호 0건)로 돌던 것은 `ksc_bars`가 낡아서였다. 복구 후 `alert` 재실행: **`데이터 기준일 2026-08-28 · 최신` → `status=ok` 신호 70건** → dispatch 204 → 브리핑 44건 완주
  - **드리프트 잡에 수동 실행을 붙였다** (상위 `6425b45`) — 토요일 cron으로만 돌게 돼 있어, 조용히 멈춘 2주를 따라잡으려면 노트북에서 돌리는 수밖에 없었다. `workflow_dispatch`에 `job` 선택(update | drift) 추가
  - **남은 것: 정기(cron) 실행 1회 확인** — 08-31 09:00 UTC분이 13:55 UTC 현재 아직 안 떴다. 깃허브 cron은 정시를 보장하지 않는다(08-28분은 09:00 예정에 20:28에 떴다). 수정된 코드로 도는 수동 실행은 성공했다(run `33350467795`)
...
- [x] **`BRIEFING_DISPATCH_TOKEN` 발급**(fine-grained PAT · `krx-signal-briefing` 1개 · Contents write) → 상위 리포 Secrets **완료 (2026-08-31)** — 절차는 `docs/TOKENS.md` §2. 실동작 확인: dispatch `HTTP 204` → 20초 안에 워크플로 시작 (run `33393160447`·`33395198128`)
...
- [x] **트리거 시험 ① `repository_dispatch`** (2026-08-31, run `33393160447`) — API 직접 호출 `HTTP 204` → **20초 안에 워크플로 시작 · 36초 만에 완주**. `event: repository_dispatch` 확인
- [x] **트리거 시험 ② 예비 cron no-op** (2026-08-31) — 실DB로 `--if-not-briefed` 확인: `브리핑이 이미 있다 — 예비 cron no-op` · 종료 0
- [x] **트리거 시험 ③ 상위 데이터가 낡은 날** (2026-08-31 일요일, run `33393160447`) — `[gate] 상위 status=stale_data data_date=2026-08-27 → stale` → **`[브리핑 없음]` 메일 발송 · `status=no_signals`**. D8대로 침묵하지 않는다. (`gate_timeout`은 상위 행 자체가 없는 경우로, 아직 안 겪었다)
- [x] **트리거 시험 ③-b `gate_timeout`** (2026-08-31) — `ksa_runs`에 행이 없는 날(`--date 20260823`, 일요일)로 재현. `[gate] 오늘 행 없음` → `[wait] 60초 대기 (1/10 … 10/10)` → **`[record_run] status=gate_timeout`(기록이 먼저)** → `finalize`에서만 raise → **종료코드 1 · 메일 없음**. 10분 소요. 설계 그대로다 (D8·N5)
...
- [x] **`README.md`(영어) · `README_KO.md`(한국어)** (2026-08-31) — v2.0 시절 내용("공시를 붙인다"·M0/M1b·32/70)을 v3.1로 다시 썼다. 상호 링크 · 그림 3장. **예시는 실제 출력**(엔투텍 08/26 — CB 4건·합산 오버행 24.04%·불일치 26점)이지 삽화가 아니다
- [x] **`docs/GRAPH.md` 최신 확인 · 설계도 3장 갱신** (2026-08-31) — `export_graph.py` 재실행 → GRAPH.md **변화 없음(최신)**. 설계도는 셋 다 낡아 있었다: `graph.dot`에 `load_market`·`publish`·`analyze` 개명이 빠짐 · `arch.dot`에 **뺀 korea-stock-mcp·KRX OPEN API**가 남고 dispatch 조건이 `if: always()`로 적혀 있음 · `modules.dot`에 `enrich`·`deploy`·`verdict`·`analysis`·`routine`·`page`가 없음. 셋 다 v3.1로 고쳐 재렌더링했고, **간선 16/16 일치**를 스크립트로 대조 확인
- [x] **워크스페이스 `CLAUDE.md` 프로젝트 표 갱신** (2026-08-31) — 상태를 M1 9/10 → M0~M3·M6 완료·M4 85%로, 목적을 "신호 검증"으로, **MCP 3종 → 2종**(korea-stock-mcp는 D14 v2에서 뺐다)으로 고쳤다
- [x] **「측정 기록」 빈칸 채움** (2026-08-31) — 아래 표의 `—` 행 4개를 실측값으로 채웠다. 「미해소 이슈」·SPEC 변경 이력은 5거래일 관찰이 끝나야 닫을 수 있어 열어 둔다
- [x] **메모리 갱신** (2026-08-31) — `supabase-shared-project-anon-scope`에 이번 `ksb_*` anon 노출을 **같은 함정의 두 번째 사례**로 추가. `krx-signal-briefing-project`를 v3.1 상태로 갱신
...
| M1 — 하루 DART 호출 수 · `020` 발생 여부 | 드라이런 9거래일 165신호에서 **DART 154회 · `020` 0회**. 하루 환산 **약 19회**(corpCode 1 + 종목당 1). 한도 20,000/일의 0.1% | 2026-08-29 |
| M1 — 규칙표 변경 내역(추가·삭제 키워드) | 실표본 3,000건으로 처음 세운 뒤 **규칙 20개로 고정**됐다 — 손검증(11/12)·드라이런·M6 본문 파싱을 거치는 동안 키워드 추가·삭제가 없었다. 바뀐 것은 매칭 방식이다: 제목 흔들림(`ㆍ`·공백·`[정정]`)을 `normalize()`로 흡수하고, F16이 정형 공시를 접고, F15가 걸린 공시의 본문을 편다 | 2026-08-31 |
| M3 — 요약 1회 입력/출력 토큰 · 비용 | v2.0(80자 요약) 15종목 1회 일괄 **입력 7,975 · 출력 992 토큰 → $0.065**(월 약 1,800원). v3.0(2,000자 근거 서술)에서 **32,100토큰 · 약 $0.34**(월 약 9,500원)로 커졌다 — 수급이 빠지면 23,479토큰 | 2026-08-31 |
...
| M4 — 상위 완료 → 브리핑 시작 지연 · 상위 메일 → 브리핑 메일 간격 | **실측 (2026-08-31)**: dispatch `HTTP 204` → 워크플로 시작 **20초**. 실제 연쇄에서는 상위 run 13:05:32 시작 → 13:07:05 종료인데 브리핑이 **13:07:03에 이미 시작**했다 — dispatch가 마지막 단계라 상위 종료 기록보다 빠르다. 브리핑 완주 13:21:41 → **두 메일 간격 약 15분**(대부분 DART fan-out과 LLM 1회) | 2026-08-31 |
