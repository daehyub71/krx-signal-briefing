...
> **상태: v2.1 확정** (2026-08-29). v1.1(D1~D11)에 **MCP 3종 활용 전환**을 §2-3으로 얹어 D12~D15를 확정했고,
> **메일 본문 레이아웃을 시안 합의로 재설계**했다 (F7 v2 · F7b 길이 예산 · N15 — 설계 근거와 시안은 `docs/DESIGN.md`).
...
| **R7** | **유사투자자문업 경계** | 해석을 남에게 배포하면 신고 대상 | `RECIPIENTS`는 **본인 한 사람** 유지 — 본인 소유 주소가 여러 개인 것은 무방하다(2026-08-29 현재 2개). **다른 사람의 주소를 넣는 순간 경계를 넘는다.** 문구는 사실만 (N1). 비목표로 못박음 |
...
| **N15** (v2.1) | **메일 한 통은 102,400 bytes를 넘지 않는다** — 넘으면 Gmail이 잘라내고 꼬리의 한계 문구(R7)까지 사라진다. `render.html()`이 예산을 넘으면 압축 카드부터 접고, 인덱스 표·위험 종목 카드·한계 문구는 접지 않는다 (F7b). `tests/test_render.py`가 실제 `EmailMessage` 크기로 확인한다 |
