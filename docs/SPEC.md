...
> **상태: v3.1 확정** (2026-08-31). **프로젝트 목적이 재정의됐다** (§2-5) — "신호에 공시를 붙인다"에서
...
| **D20** → **v2 (2026-08-31 사용자 결정)** | **메일은 더 간략하게, 전문은 Vercel 페이지로 유도** | 메일에는 인덱스 표 + 종목당 몇 줄만 싣고 상세는 링크로 보낸다. 전문 페이지는 **Vercel 정적 배포**다 — 배치가 HTML을 만들어 올리므로 **브라우저가 DB에 붙지 않는다**(anon 키를 웹 번들에 넣지 않는다; 상위 프로젝트의 anon 키 범위 이슈도 피한다). 15×2,000자=30,000자는 UTF-8 90KB로 Gmail 한계를 넘는다 |
...
| 배포 | **Vercel 정적 배포.** 배치가 HTML을 만들어 올린다 — 브라우저는 DB에 붙지 않고 키도 담기지 않는다 |
| 실패 | 페이지 생성·배포가 실패해도 **메일은 간다.** 링크 없이 발췌만 싣고 `⚠ 전문 페이지 생성 실패`를 붙인다 |

**F20b. 페이지에 반드시 있어야 하는 것** (R7 v3 · 2026-08-31 사용자 지시)

공개 URL이므로 **성격을 오해할 수 없게** 못박는다. 아래는 접히지도 흐려지지도 않는다.
...
| **R7** → **v3 (2026-08-31)** | **유사투자자문업 경계 — 이 프로젝트에서 가장 큰 리스크** | v3.0은 판정·점수·근거 서술을 낸다. **전문 페이지를 Vercel로 공개 배포하기로 하면서(D20 v2) 제공 형태가 달라졌다** — URL을 아는 사람은 누구나 열 수 있다 | ① 메일 `RECIPIENTS`는 **본인 한 사람** 유지 ② 페이지 최상단에 **"투자 권고가 아닙니다 · 참고용 테스트"**를 못박고 각 종목에도 반복 (F20b) ③ `noindex, nofollow` — 검색에 걸리지 않는다 ④ 링크를 어디에도 게시하지 않는다 ⑤ 매매 판단 금지어는 그대로(N1 v2) ⑥ 점수 한계 문구 상시 표기(F18) **한계**: 이 조치들은 오인을 줄일 뿐 공개 URL이라는 사실을 바꾸지 못한다. 불특정 다수에게 링크를 알리는 순간 경계를 넘는다 |
