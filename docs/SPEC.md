...
> **상태: v3.0 확정** (2026-08-30). **프로젝트 목적이 재정의됐다** (§2-5) — "신호에 공시를 붙인다"에서
...
| **D20** | 2,000자를 어디에 | **메일에는 발췌 2~3줄 + 「전문 보기」 링크, 전문은 웹 페이지** | 15×2,000자=30,000자는 UTF-8 90KB로 **Gmail 클리핑 한계(102,400 bytes)를 넘는다**. 메일 한 통의 물리적 상한은 종목당 약 800자 |
...
**F20. 브리핑 전문 페이지 + 메일 발췌** (v3.0, D20)

15종목 × 2,000자 = 30,000자는 UTF-8 90KB로 **Gmail 클리핑 한계(102,400 bytes)를 넘는다**.

| 어디 | 무엇 |
|------|------|
| 메일 | 인덱스 표(판정·점수 열 추가) + 종목 카드 + **분석 발췌 2~3줄** + `전문 보기 →` 링크 |
| 전문 페이지 | 종목별 2,000자 분석 전문 · 공시 본문 표 · 수급 30일 표 · 뉴스 목록 · 점수 한계 문구 |
| 접근 | **본인만 열 수 있어야 한다** (R7). 페이지를 공개 링크로 두지 않는다 |
| 실패 | 페이지 생성이 실패해도 **메일은 간다.** 링크 없이 발췌만 싣고 `⚠ 전문 페이지 생성 실패`를 붙인다 |
...
| **R7** → **v2 (v3.0)** | **유사투자자문업 경계 — v3.0에서 가장 커진 리스크** | v2.0은 사실만 나열했다. v3.0은 **판정·점수·근거 서술**을 낸다(D23·D24). 이것을 남에게 배포하면 신고 대상이다 | ① `RECIPIENTS`는 **본인 한 사람** 유지(2026-08-29 현재 본인 주소 2개) — 다른 사람 주소를 넣는 순간 경계를 넘는다 ② **전문 페이지를 공개 링크로 두지 않는다** (F20) ③ 매매 판단 금지어는 그대로 유지(N1 v2) ④ 메일·페이지에 **점수 한계 문구**를 항상 붙인다(F18) |
...
| **R22** (v3.0) | **상위 `krx-stock-charts`의 KRX 로그인이 실패 중이다** | 수급 수집이 상위에 달려 있는데, 상위 `Daily KRX update`가 2026-08-24부터 매 스케줄 실행에서 `Refresh ticker universe`(KRX 로그인)로 실패하고 있다. 로컬에서는 같은 계정으로 성공한다(2026-08-30 확인) — Actions IP 차단 의심 | **수급은 없어도 되는 층으로 설계한다**(F17 실패 처리). 상위 실패는 별도 과제로 추적 — `docs/TASKS.md` 미해소 이슈 |
