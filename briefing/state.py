...
from briefing.models import Briefing, Flow, InvestorFlows, SendResult, SignalRow
...
class FetchItem(TypedDict):
    ...
    investor_flows: InvestorFlows | None  # 기관·외국인 수급 30일 (F17). 없으면 None
...
class BriefingState(TypedDict, total=False):
    ...
    flow_skipped: str
    # ticker → 기관·외국인 수급 30일 (F17, load_market 배치 1회)
    investor_flows: dict[str, InvestorFlows]               # 시세 층 생략 사유. "" = 정상
    ...
    page_error: str                 # 전문 페이지 생성·배포 실패 사유
    summary_error: str
    llm_tokens: int
