"""신호 검증 판정과 점수 (SPEC F18, v3.0) — 순수 함수.
...
def _flag_parts(b: Briefing) -> list[Part]:
    """플래그 감산. 건수가 많아도 하한에서 멈춘다."""
    reds = sum(1 for f in b.flags if f.level == "red")
    ambers = sum(1 for f in b.flags if f.level == "amber")
...
def _body_parts(b: Briefing) -> list[Part]:
    ...
    """
    out: list[Part] = []
    overhang = sum(x.overhang_pct or 0.0 for x in b.bodies)
    if overhang > 0:
        out.append(Part(f"잠재 물량 {overhang:.2f}%", max(W_OVERHANG_CAP, -round(overhang))))
    if any(x.refix_floor is not None for x in b.bodies):
        out.append(Part("시가하락 시 전환가 조정 조항", W_REFIX))
    if any("사모" in (x.method or "") for x in b.bodies):
        out.append(Part("사모 발행", W_PRIVATE))
    return out


def _flow_parts(b: Briefing, flag_days: set[date]) -> list[Part]:
    """수급 — 기관과 외국인을 합쳐 방향만 본다. 금액의 크기는 점수에 넣지 않는다."""
    if b.flows is None or not b.flows.days:
        return []
    out: list[Part] = []
    inst, foreign = b.flows.inst_total, b.flows.foreign_total
...
def _blind_spots(b: Briefing) -> tuple[str, ...]:
    """무엇을 못 보고 낸 점수인가 (R20). 빠진 층을 앞에 둔다."""
    missing = []
    if b.flows is None or not b.flows.days:
        missing.append(LAYER_BLIND["flows"])
    if b.flags and not b.bodies:
        missing.append(LAYER_BLIND["bodies"])
    if not b.news:
        missing.append(LAYER_BLIND["news"])
    if b.anomaly is None:
        missing.append(LAYER_BLIND["anomaly"])
    return (*missing, *ALWAYS_BLIND)


def judge(b: Briefing) -> Verdict:
    """브리핑 하나를 판정한다 (F18).

    Args:
        b: 공시·본문·뉴스·수급이 채워진 브리핑.

    Returns:
        판정·점수·근거 조각·사각지대. **증거가 하나도 없으면 `무관`에 중립 점수**다 —
        모르는 것을 낮은 점수로 바꾸지 않는다.
    """
    if b.level in ("error", "unknown"):
        return Verdict(STAND_SILENT, NEUTRAL, (), _blind_spots(b))

    flag_days = {d.rcept_dt for d in b.disclosures if d.rcept_no in {f.rcept_no for f in b.flags}}
    parts: list[Part] = []
    parts.extend(_flag_parts(b))
    parts.extend(_body_parts(b))
    parts.extend(_flow_parts(b, flag_days))
    if b.anomaly is not None and (w := W_ANOMALY.get(b.anomaly.verdict, 0)):
        parts.append(Part(f"공시 이상 {b.anomaly.verdict}", w))
    if not b.flags and b.disclosures:
        parts.append(Part(f"최근 {b.window_days}일 확인된 위험 유형 없음", W_NO_RISK))
    if b.flags and b.news:
    ...
    if not parts:
        ...
        stand = STAND_SILENT
    return Verdict(stand, score, tuple(parts), _blind_spots(b))
